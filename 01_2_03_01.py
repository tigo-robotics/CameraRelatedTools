...
def compute_disparity(left_img, right_img):
    ...
    stereo = cv2.StereoSGBM_create(
        minDisparity=0,
        numDisparities=16 * 9,  # Number of disparity values
        blockSize=5,
        P1=8 * 1 * 5**2,
        P2=32 * 1 * 5**2,
        disp12MaxDiff=1,
...
def main():
    ...
    while True:
        ...
        disparity_color = cv2.applyColorMap(disparity.astype(np.uint8), cv2.COLORMAP_JET)

        # Draw horizontal lines for epipolar visualization
        for i in range(0, left_rectified.shape[0], 30):
            cv2.line(left_rectified, (0, i), (left_rectified.shape[1], i), (0, 255, 0), 1)
            cv2.line(right_rectified, (0, i), (right_rectified.shape[1], i), (0, 255, 0), 1)
